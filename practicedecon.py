...
for i = 1:50:
  if i < 10:
    ...
    darkarry(:,:,i) = np.array(dark, dtype=np.float64)
    Image.close()
  else:
...
darkmean = np.mean(darkarry,3)
background = np.std(darkarry(:,:,1)) ** 2 

decon_filename='/users/Bryan/Documents/XLXF_data/Deconvolution/results'
...
for s = 1:5:
  for p = 1:23:
    ...
    detectedimage =- darkmean
    Image.close()
    
    decon, iter_vec, diff_vec = PBRL.RL_decon(detectedimage, 0.80, pix_size, 0.15, 5, background, decon_filename, [10], stopping_condition=10e-2, maxIter_Number=10)
